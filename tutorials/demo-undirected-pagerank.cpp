...
PTable AddReverseEdgeTable() {
    TTableContext Context;
    Schema Edges;
    //Just named the first list as dstid and second one as srcid to solve the purpose
    Edges.Add(TPair<TStr, TAttrType>("DstID", atInt));
    Edges.Add(TPair<TStr, TAttrType>("SrcID", atInt));
    char filename[50];
    printf("Adding Edge Table\n");
...
void PRank_iteration(const TVec<PTable>& NTables,const TVec<PTable>& ETables,const TVec<TPair<TInt, TInt> > & Mapping, TVec<TFltV> & PRank, const TVec<TIntIntH > &NodeHash, int total_nodes) {
...
int main(int argc, char* []) {
    ...
    for (int i = 0; i < ETableCount; i++) {
        ...
        ETables.Add(AddReverseEdgeTable());
