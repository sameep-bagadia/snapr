...
void GetPagerankMM(const PTableV& NTables,const PTableV& ETables,const TIntPrV& Mapping, const TFltV Weights, const TVec<THash<TGroupKey, TPair<TInt, TIntV> > >& GroupMap, PTableV& Result) {
  TVec<TFltV> PRank;
  int NTblCnt = NTables.Len();
  int ETblCnt = ETables.Len();
  int NodeCnt = 0;
  ...
  for (int i = 0; i < IterCnt; i++) {
    ...
    IteratePRank(NTables, ETables, Mapping, Weights, GroupMap, PRank, NodeCnt);
    //PrintPRank(PRank);
  }
  /*
