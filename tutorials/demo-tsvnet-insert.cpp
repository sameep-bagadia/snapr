#include "Snap.h"
#include <sys/time.h>
#include <sys/resource.h>
#include <stdio.h>
#include <stdlib.h>     /* srand, rand */
#include <time.h>       /* time */
//#include <proc/readproc.h>

typedef TVec<PTable> PTableV;
typedef TVec<TIntPr > TIntPrV;


/**************************Benchmark Functions*****************************/

float getcputime() {
  struct rusage rusage;
  float result;
  
  getrusage(RUSAGE_SELF, &rusage);
  
  result =
  ((float) (rusage.ru_utime.tv_usec + rusage.ru_stime.tv_usec) / 1000000) +
  ((float) (rusage.ru_utime.tv_sec + rusage.ru_stime.tv_sec));
  return result;
}
/*
 void getcpumem(float *scpu, float *smem) {
 struct rusage rusage;
 struct proc_t usage;
 
 getrusage(RUSAGE_SELF, &rusage);
 *scpu =
 ((float) (rusage.ru_utime.tv_usec + rusage.ru_stime.tv_usec) / 1000000) +
 ((float) (rusage.ru_utime.tv_sec + rusage.ru_stime.tv_sec));
 
 look_up_our_self(&usage);
 *smem = (float) usage.vsize / 1000000;
 }
 
 void getmaxcpumem(float *scpu, float *smem) {
 struct rusage rusage;
 
 getrusage(RUSAGE_SELF, &rusage);
 *scpu =
 ((float) (rusage.ru_utime.tv_usec + rusage.ru_stime.tv_usec) / 1000000) +
 ((float) (rusage.ru_utime.tv_sec + rusage.ru_stime.tv_sec));
 *smem = (float) (rusage.ru_maxrss) / 1000;
 }
 */
/***************************************************************************/


//Function to read a table of nodes
PTable AddNodeTable(TTableContext& Context) {
  Schema NodeScm;
  NodeScm.Add(TPair<TStr, TAttrType>("NodeID", atStr));
  char FileName[50];
  int ColCnt = 0;
  printf("Adding Node Table\n");
  printf("Enter filename and number of columns (>= 1) \n");
  scanf("%s %d", FileName, &ColCnt);
  for (TInt i = 1; i < ColCnt; i++) {
    TStr ColName = "Attribute" + i.GetStr();
    NodeScm.Add(TPair<TStr, TAttrType>(ColName, atStr));
  }
  TStr FName(FileName);
  PTable T = TTable::LoadSS(NodeScm, FName, Context);
  return T;
}

// Function to read in a table of edges
PTable AddEdgeTable(TTableContext& Context) {
  char FileName[200];
  int ColCnt;
  int Reverse;
  printf("Adding Edge Table\n");
  printf("Enter filename, number of columns (>= 2), and whether reverse? (reverse = 1, not reverse = 0\n");
  scanf("%s %d %d", FileName, &ColCnt, &Reverse);
  Schema EdgeScm;
  if (Reverse == 1) {
    EdgeScm.Add(TPair<TStr, TAttrType>("DstID", atStr));
    EdgeScm.Add(TPair<TStr, TAttrType>("SrcID", atStr));
  }
  else {
    EdgeScm.Add(TPair<TStr, TAttrType>("SrcID", atStr));
    EdgeScm.Add(TPair<TStr, TAttrType>("DstID", atStr));
  }
  for (TInt i = 1; i < ColCnt-1; i++) {
    TStr ColName = "Attribute" + i.GetStr();
    EdgeScm.Add(TPair<TStr, TAttrType>(ColName, atStr));
  }
  TStr FName(FileName);
  PTable T = TTable::LoadSS(EdgeScm, FName, Context);
  return T;
}

void PrintBenchmarks(FILE* outfile) {
  float cputime;
  cputime = getcputime();
  /*getcpumem(&scpu, &smem);
   getmaxcpumem(&maxscpu, &maxsmem);*/
  fprintf(outfile, "%f\n", cputime);
}



int main(int argc, char* []) {
  
  FILE * outfile;
  outfile = fopen ("benchmark.txt","w");
  
  
  //outfile << "Hello World!\n";
  fprintf(outfile, "Hello World!\n");
  PrintBenchmarks(outfile);
  
  int NTblCnt;
  int ETblCnt;
  printf("Enter the number of node tables and edge tables\n");
  scanf("%d %d", &NTblCnt, &ETblCnt);
  
  TVec<PTable> NTables;
  TVec<PTable> ETables;
  TVec<PTable> OutNTables;
  TVec<TPair<TInt, TInt> > Mapping;
  //TFltV Weights;
  
  TTableContext Context;
  //Read in tables
  for (int i = 0; i < NTblCnt; i++) {
    PTable P = AddNodeTable(Context);
    P->SaveSS("ntable_abc.txt");
    NTables.Add(P);
  }
  for (int i = 0; i < ETblCnt; i++) {
    ETables.Add(AddEdgeTable(Context));
    //printf("Enter the source and destination node table number (index starting from 0), and weight of the edges\n");
    printf("Enter the source and destination node table number (index starting from 0)\n");
    int SrcId;
    int DestId;
    //float Wt;
    //scanf("%d %d %f", &SrcId, &DestId, &Wt);
    scanf("%d %d", &SrcId, &DestId);
    printf("%d %d\n", SrcId, DestId);
    Mapping.Add(TPair<TInt, TInt>(SrcId, DestId));
    //Weights.Add(Wt);
  }
  
  /*
   char nodes_file[200];
   int NodesCnt;
   printf("Enter the filename containing bfs nodes and number of nodes to take");
   scanf("%s %d", nodes_file, &NodesCnt);
   
   FILE* infile = fopen (nodes_file,"r");
   char NodeTypeStr[100];
   char NodeIdStr[200];
   TIntV NodeTypeV;
   TStrV NodeIdV;
   for (int i = 0; i < NodesCnt; i++) {
   fscanf(infile, "%s %s", NodeTypeStr, NodeIdStr);
   int NodeTypeInt;
   TStr NodeIdStr2 = TStr(NodeIdStr);
   if (NodeTypeStr[0] == 'c') { NodeTypeInt = 0; }
   else if (NodeTypeStr[0] == 'l') { NodeTypeInt = 1; }
   else if (NodeTypeStr[0] == 'p') { NodeTypeInt = 2; }
   else if (NodeTypeStr[0] == 't') { NodeTypeInt = 3; }
   else if (NodeTypeStr[0] == 'u') { NodeTypeInt = 4; }
   else { printf("ERROR! Node Type not found!\n"); }
   NodeTypeV.Add(NodeTypeInt);
   NodeIdV.Add(NodeIdStr2);
   
   }
   */
  
  fprintf(outfile, "Tables Loaded\n");
  PrintBenchmarks(outfile);
  printf("Tables Loaded\n");
  
  //Convert to Graph
  printf("Prerocessing\n");
  int itercount = 10;
  PSVNet Graph[itercount];
  TVec<TIntV> EdgeTypeV;
  TVec<TIntV> SrcNIdV;
  TVec<TIntV> DstNIdV;
  for (int iter = 0; iter < itercount; iter++) {
    Graph[iter] = new TSVNet();
    TIntV etv;
    TIntV snv;
    TIntV dnv;
    EdgeTypeV.Add(etv);
    SrcNIdV.Add(snv);
    DstNIdV.Add(dnv);
  }
  TIntV NTypeV;
  TIntV ETypeV;
  TVec<THash<TStr, TInt> > NodesHV;
  //Adding node types
  for (int i = 0; i < NTblCnt; i++) {
    for (int iter = 0; iter < itercount; iter++) {
      Graph[iter]->AddNType();
    }
    NTypeV.Add(i);
  }
  //Adding edge types
  for (int i = 0; i < ETblCnt; i++) {
    for (int iter = 0; iter < itercount; iter++) {
      Graph[iter]->AddEType(NTypeV[Mapping[i].Val1], NTypeV[Mapping[i].Val2]);
    }
    ETypeV.Add(i);
  }
  
  // Create a mapping from nodeid(string) to nodeid(int)
  for (int i = 0; i < NTblCnt; i++) {
    THash<TStr, TInt> NodesH;
    NodesHV.Add(NodesH);
    int NType = NTypeV[i];
    for (int j = 0; j < NTables[NType]->GetNumRows(); j++) {
      NodesHV[i].AddDat(NTables[NType]->GetStrVal("NodeID", j), j);
    }
  }
  // Creating the vector of edges to be added.
  
  for (int EType = 0; EType < ETblCnt; EType++) {
    int SrcNType = NTypeV[Mapping[EType].Val1];
    int DstNType = NTypeV[Mapping[EType].Val2];
    for (int i = 0; i < ETables[EType]->GetNumRows(); i++) {
      for (int iter = 0; iter < itercount; iter++) {
        EdgeTypeV[iter].Add(EType);
        SrcNIdV[iter].Add(NodesHV[SrcNType].GetDat(ETables[EType]->GetStrVal("SrcID", i)));
        DstNIdV[iter].Add(NodesHV[DstNType].GetDat(ETables[EType]->GetStrVal("DstID", i)));
      }
    }
  }
  // Shuffle the edges randomly
  /* initialize random seed: */
  int temp;
  srand (time(NULL));
  for (int iter = 0; iter < itercount; iter++) {
    
    for (int i = EdgeTypeV.Len() - 1; i > 0; i--) {
      int j = rand() % (i+1);
      
      temp = EdgeTypeV[iter][i];
      EdgeTypeV[iter][i] = EdgeTypeV[iter][j];
      EdgeTypeV[iter][j] = temp;
      
      temp = SrcNIdV[iter][i];
      SrcNIdV[iter][i] = SrcNIdV[iter][j];
      SrcNIdV[iter][j] = temp;
      
      temp = DstNIdV[iter][i];
      DstNIdV[iter][i] = DstNIdV[iter][j];
      DstNIdV[iter][j] = temp;
    }
  }
  
  
  
  
  fprintf(outfile, "Preprocessing done\n");
  PrintBenchmarks(outfile);
  //Adding edges
  int num_breaks = 10;
  //int j = 1;
  //int breakpoint = float(EdgeTypeV.Len()) / float(num_breaks);
  int chunksize = EdgeTypeV[0].Len() / num_breaks;
  printf("chunksize = %d\n", chunksize);
  for (int i = 0 ; i < num_breaks; i++) {
    for (int iter = 0; iter < itercount; iter++) {
      int start = i*chunksize;
      int end = (i+1)*chunksize;
      for (int j = start; j < end; j++) {
        Graph[iter]->AddEdge(SrcNIdV[iter][j], DstNIdV[iter][j], EdgeTypeV[iter][j]);
      }
    }
    fprintf(outfile, "Inserted %d%%\n", (i+1)*10);
    PrintBenchmarks(outfile);
    printf("Inserted %d%%\n", (i+1)*10);
  }
  /*
  while (i < EdgeTypeV.Len()) {
    if (j == chunksize) {
      fprintf(outfile, "Inserted
  for (int i = 0; i < EdgeTypeV.Len(); i++) {
    if (i == breakpoint) {
      //print benchmarks
      fprintf(outfile, "Inserted %d%%\n", j*10);
      PrintBenchmarks(outfile);
      j++;
      breakpoint = float(EdgeTypeV.Len() * j) / float(num_breaks);
    }
    Graph.AddEdge(SrcNIdV[i], DstNIdV[i], EdgeTypeV[i]);
  }
  fprintf(outfile, "Inserted %d%%\n", 100);
  PrintBenchmarks(outfile);
   */
  printf("Nodes = %d, edges = %d\n", Graph[0]->GetNodes(), Graph[0]->GetEdges());
  
  fclose(outfile);
  //fclose(infile);
  //fclose(outpr);
}


